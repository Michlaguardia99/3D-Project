    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-[#000000]`}>
      <div className="w-full flex justify-between items-center max-w-xl mx-auto">
        ...
        <div className="sm:hidden flex flex-1 justify-end items-center">
          ...
          />
          <div className={`${!toggle ? 'hidden' : 'flex'} p-6 beige-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
            <ul className="list-none flex justify-end items-start flex-col gap-4">
              {navLinks.map((link) => (
                <li
                  key={link.id}
                  className={`${active === link.title ? "text-white" : "text-secondary"
                    }fonts-poppins font-medium cursor=pointer text-[14px]`}
                  onClick={() => {
                    setToggle(!toggle);
